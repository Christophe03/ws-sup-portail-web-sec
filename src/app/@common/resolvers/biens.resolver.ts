...
export class BiensResolver {
  ...
  resolve(route: ActivatedRouteSnapshot): Observable<any> | Promise<any> | any {
    const values = [this.proprietaireList(), this.immeubleList()];
  ...
  private immeubleList(): Promise<any> {
    return new Promise((resolve, reject) => {
      this.immeubleService.findAll().subscribe((data) => {
        this.immeubles = ConstanteService.getDatas(data);
        resolve(data);
      }, reject);
    });
  }
